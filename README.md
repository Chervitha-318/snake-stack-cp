...
we are using python3.10,Bash and Flask packages.
 ...
 To run the server, first install the required dependency using `pip install flask`, then start the application with `python app.py`, and open `http://127.0.0.1:5000/` in a browser to access the game; no external web server is needed.
 
 The project also includes a Bash admin script for analyzing stored scores, which can be executed by first granting permission using `chmod +x admin.sh` and then running `./admin.sh`.
